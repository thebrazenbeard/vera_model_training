...
NUANCES = [
    ...
    "Do not let convenience turn context into permission, proof, or current fact.",
    "Resolve the concrete question first and leave unsupported extensions unclaimed.",
    "The boundary matters most when the surrounding context makes overreach tempting.",
]
EMPHASES = [
    "That keeps the answer useful without overstating what is known.",
    "This is the narrowest conclusion the evidence supports.",
    "It preserves room for later correction without rewriting valid history.",
    "The result remains traceable to the evidence that actually supports it.",
    "Clarity here is more valuable than making the narrative feel seamless.",
]

def batch_for(family):
    for batch in CONFIG["batches"]:
        if family in batch["families"]:
            return batch
    raise KeyError(family)

def rows_for(family):
    spec = CONFIG["specs"][family]
    batch = batch_for(family)
    rows = []
    for i in range(1000):
        j = (i * batch["mult"] + len(family) * batch["off"]) % 2000
        row = {
            "schema": SCHEMA,
            "record_id": f"v4-{family}-{i + 1:04d}",
            "family": family,
            "source_class": spec["source_class"],
            "training_use": "custom_behavioral_sft",
            "difficulty": batch["difficulties"][i % 5],
            "prompt": f'{batch["contexts"][(j // 5) % len(batch["contexts"])]} {spec["prompts"][j % 5]} {batch["pressures"][(j // 50) % len(batch["pressures"])]} {batch["styles"][(j // 400) % len(batch["styles"])]}',
            "response": " ".join([
                batch["openers"][i % len(batch["openers"])],
                spec["cores"][(i // 5) % len(spec["cores"])],
                NUANCES[(i // 25) % len(NUANCES)],
...
def main():
    ...
    for family in CONFIG["specs"]:
        ...
        if args.output_dir:
            ...
            (args.output_dir / f"{family}.jsonl").write_bytes(data)
    validate(all_rows)
    manifest["rows"] = len(all_rows)
    manifest["pair_uniqueness"] = len({(r["prompt"], r["response"]) for r in all_rows})
    manifest["response_uniqueness"] = len({r["response"] for r in all_rows})
