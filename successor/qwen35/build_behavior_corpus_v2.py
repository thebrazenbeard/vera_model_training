...
def run(targeted_url,gold_url):
    ...
    if len(sft)!=672: raise RuntimeError(f"SFT {len(sft)} !=672")
    ...
    if len(prefs)!=520: raise RuntimeError(f"prefs {len(prefs)} !=520")
